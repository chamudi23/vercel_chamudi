...
async function searchSpecimens(filters = {}) {
  const f = await normalizedFilters(filters); requireFilter(f, SPECIMEN_FILTER_KEYS, 'At least one specimen search filter is required.'); let query = supabase.from('specimens').select('specimen_id,skeleton_code,bone_type,side,site_name,district,province,time_period,preservation_state,location_stored,burial_context,notes,created_at')
...
async function getSpecimen(specimenId) { const result = await searchSpecimens({ specimenId, limit: 1 }); return result.records[0] ? result : notFound('specimen', specimenId) }
async function getMeasurements(specimenId) { const specimen = await getSpecimen(specimenId); if (specimen.type === 'NOT_FOUND') return specimen; const { data, error } = await supabase.from('measurements').select('measurement_id,specimen_id,bone_type,measurement_type,value,unit,notes').eq('specimen_id', text(specimenId)).limit(MAX_RESULTS); if (error) throw serviceError(error); return { type: 'MEASUREMENT_RESULTS', specimen: specimen.records[0], records: (data || []).map((row) => ({ measurementId: row.measurement_id, specimenId: row.specimen_id, boneType: row.bone_type, measurementType: row.measurement_type, value: row.value, unit: row.unit, notes: row.notes })) } }
...
async function searchImages(filters = {}) {
  ...
  if (f.skeletonCode || f.boneType || f.side) { specimenIds = await resolveSpecimenIdsForImageFilters(f); if (!specimenIds.length) return { type: 'IMAGE_RESULTS', records: [], meta: { limit: f.limit } } }
  let query = supabase.from('bone_images').select('image_id,specimen_id,image_url,file_url,bone_name,side,condition,image_view,view_angle,image_type,notes,image_notes,annotation_text,uploaded_at,created_at,specimen:specimens!bone_images_specimen_id_fkey(specimen_id,skeleton_code,bone_type,side)')
...
async function getSkeletonCoverage(skeletonCode) {
  const { data: specimens, error: specimenError } = await supabase.from('specimens').select('specimen_id,skeleton_code,bone_type,side,preservation_state').eq('skeleton_code', text(skeletonCode)).limit(500); if (specimenError) throw serviceError(specimenError); if (!(specimens || []).length) return notFound('skeleton', skeletonCode)
  const ids = specimens.map((row) => row.specimen_id); const [measurementsResult, imagesResult, module] = await Promise.all([supabase.from('measurements').select('measurement_id,specimen_id,bone_type').in('specimen_id', ids).limit(1000), supabase.from('bone_images').select('image_id,specimen_id,image_url,file_url,bone_name,condition').in('specimen_id', ids).limit(1000), coverage()]); if (measurementsResult.error || imagesResult.error) throw serviceError(measurementsResult.error || imagesResult.error)
