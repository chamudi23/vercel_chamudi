...
function nextSkeletonCode(records) {
  const highestNumber = (records || []).reduce((highest, record) => {
    const match = String(record.skeleton_code || "")
      .trim()
      .match(/^SK[-\s]?(\d+)$/i);
    return match ? Math.max(highest, Number(match[1])) : highest;
  }, 0);

  return `SK-${String(highestNumber + 1).padStart(3, "0")}`;
}

function recordsForSkeleton(records, skeletonCode) {
  const skeletonKey = normalize(skeletonCode);
  return (records || []).filter(
    (record) => normalize(record.skeleton_code) === skeletonKey,
  );
}

// A category can be stored in the specimen row or, for older records, in a measurement row.
// Convert both sources into the same category-side keys used by duplicate validation.
function occupiedCategorySideKeys(records) {
  const occupiedKeys = new Set();

  (records || []).forEach((record) => {
    specimenCategoryValues(record).forEach((value) => {
      const category = normalizeBoneCategory(value);
      if (category)
        occupiedKeys.add(categorySideKey(category.code, record.side));
    });
  });
...
function availableSidesForCategory(categoryValue, occupiedKeys) {
  const category = normalizeBoneCategory(categoryValue);
  if (!category) return [];

  return allowedSidesForCategory(categoryValue).filter(
    (side) => !occupiedKeys.has(categorySideKey(category.code, side)),
  );
}
...
function FieldError({ children }) {
  return children ? (
    <p className="mt-1 text-xs text-red-400">{children}</p>
  ) : null;
}
        ...
        return (
          <li key={name}>
            <button
              type="button"
              disabled={!complete}
              onClick={() => complete && onStepChange(step)}
              aria-current={current ? "step" : void 0}
              className={`flex w-full items-center gap-2 rounded-xl border px-3 py-2 text-left text-xs ${current ? "border-emerald-400/70 bg-emerald-500/15 text-emerald-100" : complete ? "border-emerald-500/30 bg-emerald-500/5 text-emerald-300 hover:bg-emerald-500/10" : "border-white/10 bg-white/[0.02] text-white/35"}`}
            >
              <span
                className={`flex h-5 w-5 shrink-0 items-center justify-center rounded-full text-[10px] font-bold ${complete || current ? "bg-emerald-400 text-slate-950" : "bg-white/10 text-white/60"}`}
              >
                {complete ? "\u2713" : step}
              </span>
              <span>{name}</span>
...
function ReviewSection({ title, onEdit, children }) {
  return (
    <section className="rounded-xl border border-white/10 bg-[#0f1a14]/70 p-4">
...
function ReviewField({ label, value }) {
  ...
  return (
    <div>
      ...
      <dd className="mt-0.5 break-words text-sm text-white/80">{value}</dd>
    </div>
  );
}
...
function SpecimenFormPage() {
  ...
  // Keep data preparation here so the JSX below only has to render the results.
  const skeletonCodes = useMemo(() => {
    const values = new Map();
    skeletonRecords.forEach((record) => {
      const code = record.skeleton_code?.trim();
      if (code && !values.has(normalize(code)))
        values.set(normalize(code), code);
    });
    return [...values.values()].sort((a, b) => a.localeCompare(b));
  }, [skeletonRecords]);
  const filteredSkeletonCodes = useMemo(
    () =>
      skeletonCodes.filter(
        (code) =>
          !normalize(skeletonSearch) ||
  ...
  const categoriesBySection = useMemo(
    () =>
      new Map(
        CONTROLLED_BONE_SECTIONS.map((section) => [
          section,
          CONTROLLED_BONE_CATEGORIES.filter(
  ...
  const selectedSkeletonRecords = useMemo(
    () =>
      skeletonMode === "existing" && form.skeleton_code
        ? recordsForSkeleton(skeletonRecords, form.skeleton_code)
        : [],
    [form.skeleton_code, skeletonMode, skeletonRecords],
  );
  const occupiedCategorySides = useMemo(
    () => occupiedCategorySideKeys(selectedSkeletonRecords),
    [selectedSkeletonRecords],
  );

  // A category stays visible while at least one of its valid sides is unused.
  const availableCategoriesBySection = useMemo(
    () =>
      new Map(
        CONTROLLED_BONE_SECTIONS.map((section) => [
          section,
          (categoriesBySection.get(section) || []).filter(
            (category) =>
              availableSidesForCategory(category.label, occupiedCategorySides)
                .length > 0,
          ),
        ]),
      ),
    [categoriesBySection, occupiedCategorySides],
  );
  const allowedSides = useMemo(() => {
    if (!form.bone_type) return [];
    if (skeletonMode !== "existing")
      return allowedSidesForCategory(form.bone_type);
    return availableSidesForCategory(form.bone_type, occupiedCategorySides);
  }, [form.bone_type, occupiedCategorySides, skeletonMode]);
  const sideIsLocked = allowedSides.length === 1;
  const selectedMeasurementTypes = useMemo(
    () =>
      new Set(
        measurements
          .map((row) => normalize(row.measurement_type))
          .filter(Boolean),
      ),
    [measurements],
  );
  const inputClass = (field2) =>
    `${inputBase} ${errors[field2] ? "border-red-500" : ""}`;
  const selectClass = (field2) =>
    `${inputClass(field2)} appearance-none cursor-pointer`;

  // Measurements belong to the selected bone category, so changing category
  // clears unsaved measurement rows to prevent mismatched data.
  function resetBoneMeasurements(message = "") {
    setMeasurements([emptyMeasurement()]);
    setMeasurementNotice(message);
  }

  // Switching modes starts a fresh specimen entry and clears mode-specific data.
  ...
  // Existing skeletons share site information. Conflicting values are left blank
  // so the user can review them instead of the form guessing a value.
  ...
  function handleChange(event) {
    ...
    if (name === "bone_type" && value !== form.bone_type) {
      const hasMeasurementData = measurements.some(
        (row) => row.measurement_type || row.value || row.notes,
      );
      resetBoneMeasurements(
        hasMeasurementData
      ...
      const nextSides =
        skeletonMode === "existing"
          ? availableSidesForCategory(value, occupiedCategorySides)
          : allowedSidesForCategory(value);
      setForm((previous) => ({
        ...previous,
        bone_type: value,
        side: nextSides.includes(previous.side)
          ? previous.side
          : nextSides[0] || "Unknown",
      }));
      setErrors((previous) => ({
    ...
    setErrors((previous) => ({
      ...
      [name]: "",
      ...(name === "side"
        ? { duplicateBone: "", duplicateSpecimenId: "" }
        : {}),
    }));
  }
  ...
  function handleLabChange(event) {
    ...
    setErrors((previous) => ({
      ...previous,
      [name]: "",
      ...(name.startsWith("date_range") ? { date_range: "" } : {}),
    }));
  }
  function addMeasurement() {
    if (form.bone_type)
      setMeasurements((previous) => [...previous, emptyMeasurement()]);
  }
  ...
  function handleMeasurementChange(id, field2, value) {
    setMeasurements((previous) =>
      previous.map((row) =>
        row.id === id ? { ...row, [field2]: value } : row,
      ),
    );
  ...
  function validate() {
    ...
    if (!form.specimen_id.trim())
      validationErrors.specimen_id = "Specimen ID is required.";
    if (!form.skeleton_code.trim())
      validationErrors.skeleton_code = "Skeleton Code is required.";
    if (!form.bone_type)
      validationErrors.bone_type = "Bone Category is required.";
    if (form.bone_type) {
    ...
    if (skeletonMode === "existing" && siteConflicts.length > 0)
      validationErrors.siteConflict =
        "This skeleton has conflicting Site Information. Registration is blocked until the conflict is reviewed.";
    ...
    measurements.forEach((row) => {
      ...
      if (
        ...
        validationErrors.measurements =
          "Each measurement needs a Measurement Type and a valid numeric Value.";
    });
    if ([...typeCounts.values()].some((count) => count > 1))
      validationErrors.measurements =
        "This measurement type has already been added for this specimen.";
    Object.assign(
      validationErrors,
      validateExcavationAndDating(excavation, labDating),
    );
  ...
  function goNext() {
    const keys =
      {
        1: ["specimen_id", "skeleton_code", "bone_type", "side"],
        2: ["siteConflict"],
        4: ["measurements"],
        5: [
          "depth_found",
    ...
    const stepErrors = Object.fromEntries(
      Object.entries(validate()).filter(([key]) => keys.includes(key)),
    );
  ...
  // Save the specimen and then its optional related measurement, excavation,
  ...
  async function saveSpecimen(attachImage = false) {
    ...
    if (Object.keys(validationErrors).length) {
      setErrors({
        ...validationErrors,
        validation: "Please correct the highlighted metadata fields.",
      });
      return;
    }
    setSaving(true);
    ...
    const { data: currentSkeletonRows, error: skeletonCheckError } =
      await supabase
        .from("specimens")
        .select(
          "specimen_id, skeleton_code, bone_type, side, measurements(bone_type)",
        );
    if (skeletonCheckError) {
      setErrors({
        submit: `Could not check existing skeleton records: ${skeletonCheckError.message}`,
      });
      setSaving(false);
      return;
    }
  ...
  const field = (label, name, options, placeholder = "") => (
    <div>
  ...
  const completedMeasurements = measurements.filter(
    (row) => row.measurement_type && row.value !== "",
  );

  // Each step supplies content to the shared wizard layout rendered below.
  let content;
  if (currentStep === 1)
    content = (
      <div className="space-y-5">
        <div className="inline-flex rounded-xl border border-white/10 bg-[#0f1a14] p-1">
          {[
            ...
            <button
              ...
              className={`rounded-lg px-4 py-2 text-sm font-medium ${skeletonMode === mode ? "bg-emerald-500 text-slate-950" : "text-white/50 hover:text-white"}`}
            >
              {label}
            </button>
          ))}
        ...
        <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
          <div>
            ...
            <FieldError>{errors.specimen_id}</FieldError>
          </div>
          <div>
            <label className="mb-1.5 block text-xs uppercase tracking-wider text-white/50">
              Skeleton Code *
            ...
            {skeletonMode === "existing" ? (
              <div className="space-y-2">
                ...
                <select
                  ...
                  onChange={(event) =>
                    selectExistingSkeleton(event.target.value)
                  }
                  disabled={loadingSkeletons || !skeletonCodes.length}
                  className={selectClass("skeleton_code")}
                >
                  <option value="">
                    {loadingSkeletons
                      ? "Loading Skeleton Codes..."
                      : "Select an existing Skeleton Code"}
                  </option>
                  {filteredSkeletonCodes.map((code) => (
              ...
              <input
                ...
                value={form.skeleton_code}
                readOnly
                className={`${inputClass("skeleton_code")} cursor-not-allowed opacity-75`}
              />
            )}
            <FieldError>{errors.skeleton_code}</FieldError>
          </div>
          <div>
            <label className="mb-1.5 block text-xs uppercase tracking-wider text-white/50">
              Bone Category *
            ...
            <select
              name="bone_type"
              value={form.bone_type}
              onChange={handleChange}
              className={selectClass("bone_type")}
            >
              <option value="">Select skeletal element...</option>
              {CONTROLLED_BONE_SECTIONS.map((section) => (
                <optgroup key={section} label={section}>
                  {availableCategoriesBySection.get(section).map((category) => (
            ...
            <FieldError>{errors.bone_type}</FieldError>
            {errors.duplicateBone && (
              <p className="mt-1 text-xs text-red-400">
                {errors.duplicateBone}{" "}
                {errors.duplicateSpecimenId && (
                  <button
                    type="button"
                    onClick={() =>
                      navigate(
                        `/specimens/${encodeURIComponent(errors.duplicateSpecimenId)}`,
                      )
                    }
                    className="underline"
                  >
          ...
          <div>
            ...
            <select
              ...
              value={form.side}
              onChange={handleChange}
              disabled={!form.bone_type || sideIsLocked}
              className={`${selectClass("side")} ${!form.bone_type || sideIsLocked ? "cursor-not-allowed opacity-65" : ""}`}
            >
              {!form.bone_type && (
                <option value="Unknown">Select a bone category first</option>
              )}
            ...
            {sideIsLocked && (
              <p className="mt-1 text-xs text-white/35">
                {form.bone_type === "Other"
                  ? "Anatomical side is not applicable to Other."
                  : "Midline is automatic for this category."}
              </p>
            )}
        ...
        {siteConflicts.length > 0 && (
          <div className="rounded-xl border border-red-400/30 bg-red-400/10 px-4 py-3 text-sm text-red-100">
            <p className="font-semibold">
              Conflicting Site Information was found. No value was guessed.
            </p>
            {siteConflicts.map((conflict) => (
              <p key={conflict} className="mt-1 text-xs">
                {conflict}
              </p>
            ))}
          </div>
        )}
        <FieldError>{errors.siteConflict}</FieldError>
        <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
      ...
      <div>
        <div className="mb-5 flex items-center justify-between">
          <div>
            ...
            <p className="mt-1 text-xs text-white/40">
              New rows use the bone category selected in Step 1.
            </p>
          </div>
          <button
            type="button"
            ...
            className="rounded-xl border border-emerald-500/30 bg-emerald-600/20 px-3 py-2 text-xs text-emerald-300 disabled:border-white/10 disabled:text-white/20"
          >
            + Add Row
          </button>
        </div>
        ...
        <div className="space-y-3">
          {measurements.map((row, index) => (
            <div
              key={row.id}
              className="rounded-xl border border-white/10 bg-[#0f1a14]/60 p-3"
            >
              ...
              <div className="grid grid-cols-1 gap-3 sm:grid-cols-2 lg:grid-cols-5">
                ...
                <div>
                  ...
                  <select
                    ...
                    disabled={!form.bone_type}
                    className={selectClass("measurements")}
                  >
                    ...
                    {MEASUREMENT_TYPES.map((type) => (
                      <option
                        ...
                        value={type}
                        disabled={
                          selectedMeasurementTypes.has(normalize(type)) &&
                          normalize(row.measurement_type) !== normalize(type)
                        }
                      >
                        {type}
                      </option>
                ...
                <div>
                  ...
                  <input
                    ...
                    disabled={!form.bone_type}
                    className={inputClass("measurements")}
                  />
                </div>
                <div>
                  <label className="mb-1.5 block text-xs uppercase tracking-wider text-white/50">
  ...
  if (currentStep === 5)
    content = (
      <div className="space-y-7">
        <section>
          <h3 className="mb-4 text-sm font-medium">Excavation Record</h3>
          <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
            {[
              ...
              <div key={name}>
                ...
                <input
                  ...
                  style={type === "date" ? { colorScheme: "dark" } : void 0}
                />
        ...
        <section className="border-t border-white/10 pt-6">
          <h3 className="mb-4 text-sm font-medium">Laboratory Dating</h3>
          <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
            <div>
              <label className="mb-1.5 block text-xs uppercase tracking-wider text-white/50">
                Dating Method
              ...
              <select
                name="dating_method"
                value={labDating.dating_method}
                onChange={handleLabChange}
                className={selectClass("dating_method")}
              >
                <option value="">Select method</option>
                {DATING_METHODS.map((method) => (
                  <option key={method} value={method}>
                    {method}
                  </option>
                ))}
              </select>
              <FieldError>{errors.dating_method}</FieldError>
            </div>
            {[
              ["Date Result", "date_result"],
              ["Date Range Min (BP)", "date_range_min"],
              ["Date Range Max (BP)", "date_range_max"],
              ["Lab Name", "lab_name"],
              ...
              <div key={name}>
                ...
                <FieldError>
                  {errors[name] ||
                    (name.startsWith("date_range") ? errors.date_range : "")}
                </FieldError>
              </div>
            ))}
          </div>
    ...
    content = (
      <div className="space-y-4">
        <p className="text-sm text-white/55">
          Review the registration before saving. Edit returns to that step
          without losing entries.
        </p>
        <ReviewSection title="Skeleton & Bone" onEdit={() => setCurrentStep(1)}>
          <ReviewField
            label="Skeleton Mode"
            value={
              skeletonMode === "existing" ? "Existing Skeleton" : "New Skeleton"
            }
          />
          <ReviewField label="Specimen ID" value={form.specimen_id} />
          <ReviewField label="Skeleton Code" value={form.skeleton_code} />
          <ReviewField label="Bone Category" value={form.bone_type} />
          <ReviewField label="Side" value={form.side} />
        </ReviewSection>
        <ReviewSection title="Site" onEdit={() => setCurrentStep(2)}>
          <ReviewField label="Site Name" value={form.site_name} />
          <ReviewField label="District" value={form.district} />
          <ReviewField label="Province" value={form.province} />
          <ReviewField label="Time Period" value={form.time_period} />
        </ReviewSection>
        <ReviewSection
          title="Condition & Storage"
          ...
          {completedMeasurements.length ? (
            completedMeasurements.map((row, index) => (
              <ReviewField
                ...
                value={`${form.bone_type} \u2014 ${row.measurement_type}: ${row.value} ${row.unit}${row.notes ? ` (${row.notes})` : ""}`}
              />
            ))
          ) : (
            <ReviewField label="Measurements" value="Not provided" />
          )}
        </ReviewSection>
        <ReviewSection title="Excavation" onEdit={() => setCurrentStep(5)}>
          <ReviewField
          ...
          <ReviewField
            label="Depth Found"
            value={
              excavation.depth_found === "" ? "" : `${excavation.depth_found} m`
            }
          />
          <ReviewField
            label="Excavator Name"
            value={excavation.excavator_name}
          />
          <ReviewField
            label="Excavation Notes"
            value={excavation.excavation_notes}
          />
        </ReviewSection>
        <ReviewSection
          title="Laboratory Dating"
          onEdit={() => setCurrentStep(5)}
        >
          <ReviewField label="Dating Method" value={labDating.dating_method} />
          <ReviewField label="Date Result" value={labDating.date_result} />
          <ReviewField
            label="Date Range"
            value={
              labDating.date_range_min || labDating.date_range_max
                ? `${labDating.date_range_min || "?"}\u2013${labDating.date_range_max || "?"} BP`
                : ""
            }
          />
          <ReviewField label="Lab Name" value={labDating.lab_name} />
          <ReviewField label="Result Notes" value={labDating.result_notes} />
        </ReviewSection>
      </div>
    );
  return (
    <div className="min-h-screen bg-[#0f1a14] text-white">
      <style>{`select option, select optgroup { background-color: #0f1a14; color: white; } input[type=date]::-webkit-calendar-picker-indicator { filter: invert(1); }`}</style>
      ...
      <main className="mx-auto max-w-4xl px-6 py-10">
        <div className="mb-8">
          ...
          <h1 className="text-3xl font-bold">Register Specimen</h1>
        ...
        />
        {success && (
          <div className="mb-6 rounded-xl border border-emerald-500/40 bg-emerald-500/20 px-5 py-4 text-sm text-emerald-300">
        ...
        {errors.submit && (
          <div className="mb-6 rounded-xl border border-red-500/40 bg-red-500/20 px-5 py-4 text-sm text-red-300">
            {errors.submit}
          </div>
        )}
        {errors.validation && (
        ...
        <div className="mt-6 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
          <div className="flex gap-3">
            <button
              ...
              disabled={saving}
              className="rounded-xl border border-white/10 px-5 py-2.5 text-sm text-white/45 hover:text-white"
            >
              Cancel
            </button>
            {currentStep > 1 && (
              <button
                ...
                disabled={saving}
                className="rounded-xl border border-white/10 px-5 py-2.5 text-sm text-white/75"
              >
                Previous
              </button>
            )}
          ...
          {currentStep < STEPS.length ? (
            <button
              type="button"
              onClick={goNext}
              className="rounded-xl bg-emerald-600 px-6 py-2.5 text-sm font-medium hover:bg-emerald-500"
            >
              Next
            </button>
          ) : (
            <div className="flex flex-col gap-2 sm:flex-row">
              <button
                ...
                className="rounded-xl border border-emerald-500/40 bg-emerald-500/10 px-6 py-2.5 text-sm font-medium text-emerald-200 disabled:opacity-40"
              >
                {saving && saveDestination === "record"
                  ? "Saving\u2026"
                  : "Save Specimen Only"}
              </button>
              <button
                ...
                disabled={saving || success}
                className="rounded-xl bg-emerald-600 px-6 py-2.5 text-sm font-medium disabled:bg-emerald-900 disabled:text-emerald-600"
              >
...
export { SpecimenFormPage as default };
