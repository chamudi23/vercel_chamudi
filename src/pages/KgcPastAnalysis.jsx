...
export default function PastAnalysis() {
  ...
  // Load all saved analyses from Supabase.
  useEffect(() => {
    let active = true;
  ...
  const boneTypes = useMemo(() => [...new Set(rows.map((d) => d.bonesType))].filter((v) => v && v !== '—'), [rows]);
  const locations = useMemo(() => [...new Set(rows.map((d) => d.location))].filter((v) => v && v !== '—'), [rows]);
  ...
  const filtered = useMemo(() => {
    ...
    if (boneFilter) result = result.filter((row) => row.bonesType === boneFilter);
    if (locationFilter) result = result.filter((row) => row.location === locationFilter);

    if (sortBy === 'oldest') result = [...result].sort((a, b) => a.foundDate.localeCompare(b.foundDate));
    else result = [...result].sort((a, b) => b.foundDate.localeCompare(a.foundDate));

    return result;
  }, [rows, search, boneFilter, locationFilter, sortBy]);
  ...
  return (
    <div className="max-w-6xl mx-auto">
      <SkeletalHeader title="Past Analysis" subtitle="Searchable repository of all historical case data" />
        ...
        <div className="bg-slate-800 rounded-xl border border-slate-700 overflow-hidden">
          <div className="px-6 py-4 border-b border-slate-700 flex items-center justify-between">
            ...
            <span className="text-slate-500 text-sm">
              {loading ? 'Loading…' : `${filtered.length} of ${rows.length} records`}
            </span>
          </div>
                ...
                {loading ? (
                  <tr>
                    <td colSpan={columns.length + 1} className="px-6 py-12 text-center text-slate-500">
                      Loading cases from the database…
                    </td>
                  </tr>
                ) : filtered.length === 0 ? (
                  <tr>
                    <td colSpan={columns.length + 1} className="px-6 py-12 text-center">
                      <div className="text-slate-500">
                        ...
                        <p className="font-medium">{rows.length === 0 ? 'No analyses yet' : 'No cases found'}</p>
                        <p className="text-sm mt-1">{rows.length === 0 ? 'Run a new analysis to see it here.' : 'Try adjusting your search or filters'}</p>
                      </div>
                    </td>
                  </tr>
                ) : filtered.map((row, i) => (
